...
run('optionCountForItems returns the right option bands', () => {
...
run('buildFallbackOptions only uses selected wardrobe ids', () => {
...
run('buildFallbackChat keeps the wardrobe context', () => {
...
run('parseJsonFromText extracts JSON from wrapped model output', () => {
  const parsed = parseJsonFromText('Here you go:\n{"reply":"Test","summary":"Event"}\nThanks.');

  assert.deepEqual(parsed, { reply: 'Test', summary: 'Event' });
});

run('buildFallbackImage always returns a renderable data URL', () => {
...
run('buildFallbackIdentification infers wardrobe traits from the upload name', () => {
...
run('buildFallbackIdentification respects existing wardrobe context when available', () => {
  const identification = buildFallbackIdentification({
